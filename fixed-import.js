...
async function createAsset(serialNumber, department, customerId) {
  try {
    ...
    log.success(`Created Asset: ${serial} (Model: ${model}) for customer ${customerId}`);
    stats.assetsCreated++;
    return asset;
...
async function processRow(row, index) {
  try {
    ...
    if (!customerName || !zone) {
      log.error(`Row ${index}: Missing required data - Customer: "${customerName}", Zone: "${zone}"`);
      stats.errors++;
      return false;
    }
